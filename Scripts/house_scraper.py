...
class HouseDisclosureScraper:
    ...
    def load_scraped_filings(self, filepath: str = None) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"No scraped filings file found at {filepath}")
            return []
        except Exception as e:
...
def main():
    ...
    scraper = HouseDisclosureScraper(headless=False)  # Set to True for production
    
    # Scrape PTR filings for 2025
    filings = scraper.scrape_ptr_filings(year=2025, max_pages=5)  # Limit to 5 pages for testing
    
    # Save results
    scraper.save_scraped_filings()
    
    # Print summary
    print(f"\nScraped {len(filings)} PTR filings")
    if filings:
        print("\nFirst 5 filings:")
        for filing in filings[:5]:
            print(f"  - {filing['doc_id']}: {filing['member_name']} ({filing['filing_type']})")
