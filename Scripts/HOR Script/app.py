...
DELETED_DOCS_FILE = os.path.join(os.path.dirname(__file__), 'hor_deleted_docs.json')
...
from db_processor import get_db_connection, create_tables
    ...
    while True:
        try:
            ...
            try:
                ...
                member_data = {'DocID': doc_id, 'First': member_name.split()[0], 'Last': member_name.split()[-1]}
...
def get_longest_transactions():
    ...
    try:
        ...
        logging.warning("length_limit_failures.json not found")
    except json.JSONDecodeError:
        logging.warning("Error decoding length_limit_failures.json")
...
if __name__ == '__main__':
    app.run(debug=True, port=5000, threaded=False)  # Disable threading for easier debugging 