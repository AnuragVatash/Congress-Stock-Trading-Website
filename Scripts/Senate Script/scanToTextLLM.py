...
from senate_db_processor import get_db_connection, get_existing_doc_ids
...
def save_text_extraction_failures():
    ...
    try:
        ...
        logging.info(f"Saved {len(text_extraction_failures)} Senate text extraction failures")
...
def load_text_extraction_failures():
    ...
    try:
        ...
        logging.info(f"Loaded {len(text_extraction_failures)} Senate text extraction failures")
    except FileNotFoundError:
        logging.info("No existing Senate text extraction failures file found")
    except Exception as e:
...
def save_generation_ids():
    ...
    try:
        filepath = os.path.join(os.path.dirname(__file__), 'senate_generation_ids.json')
        with open(filepath, 'w') as f:
            json.dump(generation_ids, f)
        logging.info(f"Saved {len(generation_ids)} Senate generation IDs")
    except Exception as e:
...
def load_generation_ids():
    ...
    try:
        ...
        logging.info(f"Loaded {len(generation_ids)} Senate generation IDs")
    except FileNotFoundError:
        logging.info("No existing Senate generation IDs file found")
    except Exception as e:
...
def save_length_limit_failures():
    ...
    try:
        ...
        logging.info(f"Saved {len(length_limit_failures)} Senate length limit failures")
    except Exception as e:
...
def load_length_limit_failures():
    ...
    try:
        ...
        logging.info(f"Loaded {len(length_limit_failures)} Senate length limit failures")
    except FileNotFoundError:
        logging.info("No existing Senate length limit failures file found")
    except Exception as e:
...
def parse_llm_transactions(text: str, member_data: dict) -> list:
    ...
    for i, line_content in enumerate(lines):
        ...
        try:
            ...
            # 4. Basic Date Validation and Fix for Missing Notification Dates
            ...
            if not notification_date_str or not notification_date_str.strip():
                logging.info(f"[{doc_id}] Empty notification date, defaulting to transaction date for: {original_line_for_logging}")
                notification_date_str = transaction_date_str
            ...
            if not re.match(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', notification_date_str):
                ...
                # If notification date is still invalid after defaulting, use transaction date
                notification_date_str = transaction_date_str
                logging.info(f"[{doc_id}] Defaulting notification date to transaction date: {transaction_date_str}")
            ...
            error_filepath = os.path.join(os.path.dirname(__file__), "senate_llm_parse_errors.txt")
...
def call_llm_api_with_text(raw_text: str, member_data: dict):
    """
    Calls OpenRouter API directly with extracted text (for table-based documents).
    Bypasses PDF processing since we already have the text content.
    Args:
        raw_text (str): The extracted text content from the document.
        member_data (dict): Metadata about the filer.
    Returns:
        str: The raw LLM output containing CSV formatted transactions or error message.
    """
    ...
    doc_id = member_data.get('DocID', 'Unknown DocID')
    llm_output = ""
    pdf_link = member_data.get('URL', 'Table-based document')
    ...
    try:
        ...
        if not filing_id:
            logging.info(f"[{doc_id}] Creating filing record for table-based document")
            # Get member name from member_data
    ...
    try:
        ...
        if not raw_text.strip():
            logging.warning(f"[{doc_id}] No text content provided for table-based document.")
            return "NO_TEXT_FOUND"

        logging.info(f"[{doc_id}] Processing table-based document with {len(raw_text)} characters of text")
...
"""
        ...
        headers = {
            ...
            "Content-Type": "application/json",
            "HTTP-Referer": "http://localhost:3000",
            "X-Title": "Senate Committee Financial Disclosures Scraper"
        }

        logging.info(f"[{doc_id}] Sending table text to OpenRouter API with max_tokens={max_tokens}...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "choices" in response_data and len(response_data["choices"]) > 0:
                    ...
                    # Store API request information
                    if filing_id:
                        logging.info(f"[{doc_id}] Storing successful table-based API request in database")
                        cursor.execute('''
                            INSERT INTO API_Requests (
                                filing_id, doc_id, generation_id, model, max_tokens,
                                text_length, approx_tokens, finish_reason, response_status,
                        ...
                        conn.commit()
                        logging.info(f"[{doc_id}] Successfully stored table-based API request")
                    else:
                        logging.warning(f"[{doc_id}] No filing_id found for table-based API request")
                    
                    logging.info(f"[{doc_id}] Received CSV response from table text:\n{csv_text[:200]}...")
                    
                    # Store the raw LLM output
                    llm_output = csv_text
                    logging.info(f"[{doc_id}] Successfully received LLM response for table-based document.")
                else:
                    error_detail = response_data.get('error', {}).get('message', 'No content in choices.')
                    logging.error(f"[{doc_id}] No valid response content for table-based document. Detail: {error_detail}")
                    llm_output = "DOCUMENT_UNREADABLE"
                    
                    # Store failed API request
                    if filing_id:
                        logging.info(f"[{doc_id}] Storing failed table-based API request in database")
                        cursor.execute('''
                            INSERT INTO API_Requests (
                                filing_id, doc_id, model, max_tokens,
                                text_length, approx_tokens, response_status, error_message,
                        ...
                        conn.commit()
                        logging.info(f"[{doc_id}] Successfully stored failed table-based API request")
                    else:
                        logging.warning(f"[{doc_id}] No filing_id found for failed table-based API request")
            else:
                logging.error(f"[{doc_id}] Error calling OpenRouter API for table-based document: {response.status_code} - {response.text}")
                llm_output = "DOCUMENT_UNREADABLE"
                
                # Store failed API request
                if filing_id:
                    logging.info(f"[{doc_id}] Storing table-based API error in database")
                    cursor.execute('''
                        INSERT INTO API_Requests (
                            filing_id, doc_id, model, max_tokens,
                            text_length, approx_tokens, response_status, error_message,
                    ...
                    conn.commit()
                    logging.info(f"[{doc_id}] Successfully stored table-based API error")
                else:
                    logging.warning(f"[{doc_id}] No filing_id found for table-based API error")

        except requests.exceptions.Timeout:
            logging.error(f"[{doc_id}] Timeout error during table-based OpenRouter request.")
            llm_output = "DOCUMENT_UNREADABLE"
            _store_error_in_db(doc_id, "Timeout error", pdf_link, raw_text, "DOCUMENT_UNREADABLE")
                
        except requests.exceptions.RequestException as req_err:
            logging.error(f"[{doc_id}] Network error during table-based OpenRouter request: {req_err}")
            llm_output = "DOCUMENT_UNREADABLE"
            _store_error_in_db(doc_id, f"Network error: {str(req_err)}", pdf_link, raw_text, "DOCUMENT_UNREADABLE")
        ...
        logging.error(f"[{doc_id}] Error processing table-based document with OpenRouter API: {e}", exc_info=True)
        llm_output = "DOCUMENT_UNREADABLE"
        _store_error_in_db(doc_id, f"Error processing table-based document: {str(e)}", pdf_link, raw_text, "DOCUMENT_UNREADABLE")
